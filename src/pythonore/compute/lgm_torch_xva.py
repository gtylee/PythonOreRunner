...
from .lgm_ir_options import CapFloorDef, _fixing_times
...
def discount_bond_paths_torch(
    ...
    T_np = _clamp_nonnegative_times(_to_numpy_float_array(maturities))
    h_t = float(model.H(t_eff))
    z_t = float(model.zeta(t_eff))
...
def discount_bond_path_grid_torch(
    ...
    t_np = _clamp_nonnegative_times(_to_numpy_float_array(eval_times))
    T_np = _clamp_nonnegative_times(_to_numpy_float_array(maturities))
    h_t = torch.as_tensor(np.asarray(model.H(t_np), dtype=float), dtype=x.dtype, device=x.device)
...
def forward_rate_from_bonds_torch(
    model: LGM1F,
    disc_curve: TorchDiscountCurve,
    fwd_curve: TorchDiscountCurve,
    t: float,
    ...
    bt = fwd_curve.discount(float(t)) / disc_curve.discount(float(t))
    bs = fwd_curve.discount(s) / disc_curve.discount(s)
    be = fwd_curve.discount(e) / disc_curve.discount(e)
...
def swap_npv_from_ore_legs_dual_curve_torch(
    ...
    *,
    ...
    if torch.any(live):
        ...
        spread = torch.nan_to_num(legs_t["float_spread"][live], nan=0.0, posinf=0.0, neginf=0.0)
        ...
        if torch.any(fixed):
            ...
            coupon_fix = torch.nan_to_num(coupon_fix, nan=0.0, posinf=0.0, neginf=0.0)
...
def deflate_lgm_npv_paths_torch(
    ...
    *,
    ...
    torch = _require_torch()
    with torch.inference_mode():
        t = _clamp_nonnegative_times(times)
...
def swap_npv_paths_from_ore_legs_dual_curve_torch(
    ...
    *,
    ...
    torch = _require_torch()
    with torch.inference_mode():
        eval_times_np = _clamp_nonnegative_times(times)
        x = torch.as_tensor(x_paths, dtype=disc_curve.dtype, device=disc_curve.device_obj)
        ...
        fixed_pay_np = _clamp_nonnegative_times(legs.get("fixed_pay_time", []))
        float_pay_np = _clamp_nonnegative_times(legs.get("float_pay_time", []))
        float_start_np = _clamp_nonnegative_times(legs.get("float_start_time", []))
        float_end_np = _clamp_nonnegative_times(legs.get("float_end_time", []))
    ...
    if fixed_pay.numel() > 0:
        fixed_start = legs_t.get("fixed_start_time")
        if exercise_into_whole_periods and fixed_start is not None:
    ...
    if pay.numel() > 0:
        ...
        spread = torch.nan_to_num(legs_t["float_spread"], nan=0.0, posinf=0.0, neginf=0.0)
        ...
        if deterministic_fixings_cutoff is None:
            ...
            cutoff = torch.as_tensor(float(deterministic_fixings_cutoff), dtype=x.dtype, device=x.device)
            fixed = fix_t[None, :] <= cutoff + 1.0e-12
        ...
        if torch.any(live & fixed):
            ...
            coupons = torch.nan_to_num(coupons, nan=0.0, posinf=0.0, neginf=0.0)
...
def price_plain_rate_leg_paths_torch(
    ...
    *,
    ...
    with torch.inference_mode():
        ...
        start = torch.as_tensor(_clamp_nonnegative_times(start_np), dtype=x.dtype, device=x.device)
        end = torch.as_tensor(_clamp_nonnegative_times(end_np), dtype=x.dtype, device=x.device)
        fixing = torch.as_tensor(_clamp_nonnegative_times(fixing_np), dtype=x.dtype, device=x.device)
        ...
        spread = torch.nan_to_num(torch.as_tensor(spread_np, dtype=x.dtype, device=x.device), nan=0.0, posinf=0.0, neginf=0.0)
        ...
        amount = sign * notional[None, :, None] * accr[None, :, None] * coupon
    ...
    *,
    ...
    with torch.inference_mode():
        ...
        p_start = discount_bond_paths_torch(
            ...
            x,
            p_t,
            curve.discount(torch.as_tensor([effective_start], dtype=x.dtype, device=x.device)),
        )[0]
        p_end = discount_bond_paths_torch(
            ...
            x,
            p_t,
            curve.discount(torch.as_tensor([maturity], dtype=x.dtype, device=x.device)),
        )[0]
        ...
        for pay in pay_times:
            ...
            disc = discount_bond_paths_torch(
                ...
                x,
                p_t,
                curve.discount(torch.as_tensor([float(pay)], dtype=x.dtype, device=x.device)),
            )[0]
    ...
    with torch.inference_mode():
        ...
        if np.any(fixed_mask):
            ...
            disc_fix = discount_bond_paths_torch(
                ...
                x,
                p_t,
                disc_curve.discount(torch.as_tensor(p[fixed_mask], dtype=x.dtype, device=x.device)),
            )
