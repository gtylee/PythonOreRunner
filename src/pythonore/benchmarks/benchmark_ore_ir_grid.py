...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    p.add_argument("--forward-column", default="EUR-EONIA")
...
def _ore_pricing_only_xml(output_dir: Path, input_dir: Path) -> str:
    return f"""<?xml version="1.0"?>
<ORE>
  <Setup>
...
def _render_report(rows: list[GridRow], ore_seconds: float, *, discount_column: str, forward_column: str, model_source: str, alpha_scale: float, kappa_override: float | None) -> str:
    table_rows = [
        (
            row.trade_id,
            _fmt_num(row.ore_pv),
            _fmt_num(row.py_pv_single),
            _fmt_pct((row.py_pv_single - row.ore_pv) / max(abs(row.ore_pv), 1.0)),
            _fmt_num(row.py_pv_dual6m),
            _fmt_pct((row.py_pv_dual6m - row.ore_pv) / max(abs(row.ore_pv), 1.0)),
            row.model_source,
        )
        for row in rows
    ]
    lines = [
        ...
        "",
        _make_table(
            ["trade_id", "ORE_PV", "PY_single", "single_rel", "PY_dual6m", "dual6m_rel", "model_source"],
            table_rows,
        ),
    ]
    return "\n".join(lines).rstrip() + "\n"
...
def main() -> None:
    ...
    _write(inp / "ore.xml", _ore_pricing_only_xml(out, inp))
    ...
    ore_seconds = perf_counter() - t0
    if proc.returncode != 0:
        raise RuntimeError(f"ORE pricing-only run failed:\n{proc.stdout}\n{proc.stderr}")
    ...
    if not (curves_csv.exists() and flows_csv.exists() and npv_csv.exists() and calibration_xml.exists()):
        raise FileNotFoundError("expected pricing/calibration outputs missing")

    base_model = _build_model_from_ore_calibration(calibration_xml, ccy_key="EUR") if args.model_source == "classic_calibration" else _build_simulation_stub_model()
    ...
    disc_t, disc_df = load_ore_discount_pairs_from_curves(curves_csv.as_posix(), discount_column=args.discount_column)
    p0_disc = build_discount_curve_from_discount_pairs(list(zip(disc_t.tolist(), disc_df.tolist())))
    fwd_t, fwd_df = load_ore_discount_pairs_from_curves(curves_csv.as_posix(), discount_column=args.forward_column)
    ...
    for spec in BERM_TRADE_SPECS:
        ...
        berm = _build_berm_from_ore_flows(flows_csv, asof=date(2016, 2, 5), trade_id=trade_id)
        rows.append(
            GridRow(
                trade_id=trade_id,
                ore_pv=_load_trade_npv(npv_csv, trade_id),
                py_pv_single=float(bermudan_backward_price(model, p0_disc, p0_disc, berm, n_grid=BERM_BACKWARD_N_GRID).price),
                py_pv_dual6m=float(bermudan_backward_price(model, p0_disc, p0_fwd, berm, n_grid=BERM_BACKWARD_N_GRID).price),
