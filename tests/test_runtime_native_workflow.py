...
def _generic_averaged_overnight_rate_swap_trade_xml(*, ccy: str = "USD", index: str = "USD-SOFR", rate_cutoff: int = 0) -> str:
...
def test_torch_averaged_overnight_with_cutoff_matches_numpy_runtime(ccy, index_name, quotes):
    pytest.importorskip("torch")
    snapshot = _make_snapshot()
    trade = Trade(
        trade_id=f"AVG_{ccy}_OIS_CUTOFF_TORCH_PARITY",
        counterparty="CP_A",
        netting_set="NS_AVG_OIS_CUTOFF",
        trade_type="Swap",
        product=GenericProduct(
            payload={
                "trade_type": "Swap",
                "xml": _generic_averaged_overnight_rate_swap_trade_xml(ccy=ccy, index=index_name, rate_cutoff=2),
            }
        ),
    )
    snapshot = replace(
        ...
        netting=NettingConfig(
            netting_sets={
                "NS_AVG_OIS_CUTOFF": NettingSet(
                    netting_set_id="NS_AVG_OIS_CUTOFF",
                    counterparty="CP_A",
                    active_csa=True,
                    csa_currency=ccy,
                    threshold_receive=0.0,
        ...
        collateral=CollateralConfig(
            balances=(CollateralBalance(netting_set_id="NS_AVG_OIS_CUTOFF", currency=ccy),)
        ),
        config=replace(
            snapshot.config,
            base_currency=ccy,
    ...
    numpy_adapter = XVAEngine.python_lgm_default(fallback_to_swig=False).adapter
    with patch.object(numpy_adapter, "_resolve_irs_pricing_backend", return_value=None):
        numpy_result = numpy_adapter.run(snapshot, mapped=mapped, run_id=f"avg-{ccy.lower()}-ois-cutoff-numpy")

    torch_adapter = XVAEngine.python_lgm_default(fallback_to_swig=False).adapter
    with patch.object(torch_adapter, "_resolve_irs_pricing_backend", return_value=_torch_irs_backend()):
        torch_result = torch_adapter.run(snapshot, mapped=mapped, run_id=f"avg-{ccy.lower()}-ois-cutoff-torch")
...
def test_torch_averaged_sifma_cutoff_rate_swap_uses_bma_curve_and_matches_numpy_runtime():
    pytest.importorskip("torch")
    snapshot = _make_snapshot()
    trade = Trade(
        trade_id="AVG_SIFMA_CUTOFF_TORCH_PARITY",
        counterparty="CP_A",
        netting_set="NS_SIFMA_AVG",
        trade_type="Swap",
        product=GenericProduct(
            payload={
                "trade_type": "Swap",
                "xml": _generic_sifma_fixed_float_rate_swap_trade_xml(rate_cutoff=2, is_averaged=True),
            }
        ),
    )
    snapshot = replace(
        ...
        netting=NettingConfig(
            netting_sets={
                "NS_SIFMA_AVG": NettingSet(
                    netting_set_id="NS_SIFMA_AVG",
                    counterparty="CP_A",
                    active_csa=True,
                    csa_currency="USD",
                    threshold_receive=0.0,
        ...
        collateral=CollateralConfig(
            balances=(CollateralBalance(netting_set_id="NS_SIFMA_AVG", currency="USD"),)
        ),
        config=replace(
            snapshot.config,
            base_currency="USD",
    ...
    numpy_adapter = XVAEngine.python_lgm_default(fallback_to_swig=False).adapter
    with patch.object(numpy_adapter, "_resolve_irs_pricing_backend", return_value=None):
        numpy_result = numpy_adapter.run(snapshot, mapped=mapped, run_id="avg-sifma-cutoff-numpy")

    torch_adapter = XVAEngine.python_lgm_default(fallback_to_swig=False).adapter
    with patch.object(torch_adapter, "_resolve_irs_pricing_backend", return_value=_torch_irs_backend()):
        torch_result = torch_adapter.run(snapshot, mapped=mapped, run_id="avg-sifma-cutoff-torch")
...
@pytest.mark.parametrize(
    ...
    (
        (
            "USD_LIB3M_LIB6M_BASIS_TORCH",
            _generic_float_float_basis_swap_trade_xml(),
            (
                MarketQuote(date="2026-03-08", key="ZERO/RATE/USD/2Y", value=0.0300),
                MarketQuote(date="2026-03-08", key="IR_SWAP/RATE/USD/USD-LIBOR-3M/1Y/2Y", value=0.0310),
                MarketQuote(date="2026-03-08", key="IR_SWAP/RATE/USD/USD-LIBOR-6M/1Y/2Y", value=0.0320),
            ),
        ),
        (
            "USD_SOFR3M_LIB3M_BASIS_TORCH",
            _generic_float_float_basis_swap_trade_xml(index0="USD-SOFR-3M", index1="USD-LIBOR-3M", tenor0="3M", tenor1="3M"),
            (
                MarketQuote(date="2026-03-08", key="ZERO/RATE/USD/2Y", value=0.0300),
                MarketQuote(date="2026-03-08", key="IR_SWAP/RATE/USD/USD-SOFR-3M/1Y/2Y", value=0.0305),
                MarketQuote(date="2026-03-08", key="IR_SWAP/RATE/USD/USD-LIBOR-3M/1Y/2Y", value=0.0310),
            ),
        ),
    ),
)
def test_torch_float_float_basis_swap_matches_numpy_runtime(trade_id, xml, quotes):
    pytest.importorskip("torch")
    snapshot = _make_snapshot()
    trade = Trade(
        trade_id=trade_id,
        counterparty="CP_A",
        netting_set="NS_USD_BASIS",
        trade_type="Swap",
        product=GenericProduct(payload={"trade_type": "Swap", "xml": xml}),
    )
    snapshot = replace(
        snapshot,
        market=replace(
            snapshot.market,
            raw_quotes=tuple(snapshot.market.raw_quotes) + tuple(quotes),
        ),
        portfolio=replace(snapshot.portfolio, trades=(trade,)),
        netting=NettingConfig(
            netting_sets={
                "NS_USD_BASIS": NettingSet(
                    netting_set_id="NS_USD_BASIS",
                    counterparty="CP_A",
                    active_csa=True,
                    csa_currency="USD",
                    threshold_receive=0.0,
        ...
        collateral=CollateralConfig(
            balances=(CollateralBalance(netting_set_id="NS_USD_BASIS", currency="USD"),)
        ),
        config=replace(
            snapshot.config,
            base_currency="USD",
    ...
    np.testing.assert_allclose(
        np.asarray(torch_result.cubes["npv_cube"].payload[trade_id]["npv_paths"], dtype=float),
        np.asarray(numpy_result.cubes["npv_cube"].payload[trade_id]["npv_paths"], dtype=float),
        rtol=0.0,
        atol=1.0e-8,
    )
    _assert_numpy_safe_result_arrays(torch_result)
...
def test_torch_rate_swap_exclusions_are_specific_for_non_plain_conventions():
    ...
    averaged_cutoff_spec = _TradeSpec(
        trade=base_trade,
        kind="RateSwap",
        notional=1_000_000.0,
        ccy="USD",
        legs={
            "rate_legs": [
                fixed_leg,
                {**vanilla_float, "index_name": "USD-SOFR", "overnight_indexed": True, "is_averaged": True, "rate_cutoff": 2},
            ]
        },
    )
    assert adapter._torch_rate_swap_exclusion_reasons(averaged_cutoff_spec) == ()
    assert adapter._supports_torch_rate_swap(averaged_cutoff_spec)
    ...
    assert adapter._torch_rate_swap_exclusion_reasons(basis_spec) == ()
    assert adapter._supports_torch_rate_swap(basis_spec)

    sifma_basis_spec = _TradeSpec(
        trade=base_trade,
        kind="RateSwap",
        notional=1_000_000.0,
        ccy="USD",
        legs={"rate_legs": [vanilla_float, {**vanilla_float, "index_name": "USD-SIFMA-1W", "overnight_indexed": True}]},
    )
    ...
    assert adapter._torch_rate_swap_exclusion_reasons(averaged_sifma_spec) == ()
    assert adapter._supports_torch_rate_swap(averaged_sifma_spec)

    averaged_sifma_cutoff_spec = _TradeSpec(
        trade=base_trade,
        kind="RateSwap",
        notional=1_000_000.0,
        ccy="USD",
        legs={
            "rate_legs": [
                fixed_leg,
                {**vanilla_float, "index_name": "USD-SIFMA-1W", "overnight_indexed": True, "is_averaged": True, "rate_cutoff": 2},
            ]
        },
    )
    assert adapter._torch_rate_swap_exclusion_reasons(averaged_sifma_cutoff_spec) == ()
    assert adapter._supports_torch_rate_swap(averaged_sifma_cutoff_spec)
