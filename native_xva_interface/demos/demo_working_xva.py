...
def main() -> None:
    ...
    # Programmatic market/portfolio on top of known-good XVA config buffers.
    ...
    if os.getenv("RUN_ORE_SWIG_DEMOS") != "1":
        print("Skipping ORE-SWIG demo run (set RUN_ORE_SWIG_DEMOS=1 to enable).")
        return

    # Keep this demo fast by capping runtime samples.
    base.config.num_paths = 64
    try:
        result = XVAEngine(adapter=ORESwigAdapter()).create_session(snap).run(return_cubes=False)
    ...
    print("reports:", sorted(result.reports.keys())[:20])
